...
try:
    ...
    if connection.is_connected():
        print("The connection to the MySQL database was succesful.")
...
def read_data_committee():
    ...
    for committee in committees:
        print(
            f"CommitteeID: {committee[0]}, CommitteeName: {committee[1]}, Treasurer: {committee[2]}, CommitteeType: {committee[3]}, State: {committee[4]}")
...
def read_data_candidate():
    ...
    for candidate in candidates:
        print(
            f"CandidateID: {candidate[0]}, CandidateName: {candidate[1]}, PartyAffiliation: {candidate[2]}, Office: {candidate[3]}, State: {candidate[4]}, CommitteeID: {candidate[5]}, ElectionID: {candidate[6]}")
...
def update_data_candidate(candidate_id, candidate_name, party_affiliation, office, state, committee_id, election_id):
    ...
    try:
        ...
        cursor.execute(update_query,
                       (candidate_name, party_affiliation, office, state, committee_id, election_id, candidate_id))
        # Commit the changes to the database
        connection.commit()
...
def read_data_contribution():
    ...
    for contribution in contributions:
        print(
            f"ContributionID: {contribution[0]}, DonorName: {contribution[1]}, CommitteeID: {contribution[2]}, Amount: {contribution[3]}, Date: {contribution[4]}")
...
def read_data_expenditure():
    ...
    for expenditure in expenditures:
        print(
            f"ExpenditureID: {expenditure[0]}, Payee: {expenditure[1]}, CommitteeID: {expenditure[2]}, Amount: {expenditure[3]}, Purpose: {expenditure[4]}, Date: {expenditure[5]}")
...
def read_data_filing():
    ...
    for filing in filings:
        print(
            f"FilingID: {filing[0]}, ReportType: {filing[1]}, ReportPeriod: {filing[2]}, DateFiled: {filing[3]}, CommitteeID: {filing[4]}")
...
def perform_action(table_choice, action_choice):
    ...
    if action_name == 'update_data':
        if table_name == 'election':
            ...
            committee_id = input("Enter Committee ID: ")
            election_id = input("Enter Election ID: ")
            update_data_candidate(int(candidate_id), candidate_name, party_affiliation, office, state,
                                  int(committee_id), int(election_id))
            return
